...
export const AuthScreen: React.FC<AuthScreenProps> = ({
  ...
  const handleSignupImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error('Signup image reading error:', err);
      setErrorMsg(err?.message || (lang === 'bn' ? 'ছবি প্রসেস করা যায়নি' : 'Failed to process photo'));
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        const result = await signUpUser(phone, password, displayName, email, signupPhotoURL);
        if (result.success && result.user) {
          onAuthSuccess(result.user);
        } else {
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl dark:shadow-blue-500/5 backdrop-blur-xl relative z-10 space-y-6">
        ...
        {mode !== 'forgot_password' && (
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && (
              <>
                {/* Profile Photo Upload Field for Signup */}
                <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl flex items-center gap-3">
                  <div className="relative shrink-0">
                    <img
                      ...
                      alt="Avatar"
                      className="w-12 h-12 rounded-full object-cover ring-2 ring-blue-500 shadow-sm"
                    />
                    {isProcessingPhoto && (
                      <div className="absolute inset-0 bg-black/60 rounded-full flex items-center justify-center text-blue-400">
                  ...
                  <div className="flex-1 space-y-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className="px-2.5 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-[10px] font-bold flex items-center gap-1 shadow-xs cursor-pointer transition-colors"
                      >
                        <Upload className="w-3 h-3" />
                        <span>{lang === 'bn' ? 'ফোন থেকে ছবি দিন' : 'Select Photo'}</span>
                      </button>

                      <button
                        type="button"
                        onClick={() => setShowCameraModal(true)}
                        className="px-2.5 py-1 bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-xl text-[10px] font-semibold flex items-center gap-1 cursor-pointer transition-colors"
                      >
      ...
      {imageToCrop && (
        <ImageCropModal
          ...
          onCropComplete={(croppedDataUrl) => {
            setSignupPhotoURL(croppedDataUrl);
            setImageToCrop(null);
          }}
